...
<?php
  $rs = mysqli_query($conn, "SELECT `Код_номера`, `Тип_номера`, `Статус` FROM `rooms` WHERE `Статус` = 'занят'"); 
  $rs2 = mysqli_query($conn, "SELECT `Код_номера`, `Тип_номера`, `Статус` FROM `rooms` WHERE `Статус` = 'занят'");
